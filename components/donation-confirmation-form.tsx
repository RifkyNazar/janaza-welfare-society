...
export function DonationConfirmationForm() {
  ...
  return <form action={action} className="rounded-3xl border border-border bg-white p-6 shadow-[0_20px_60px_rgba(16,42,42,0.07)] sm:p-8">
    <div className="grid gap-6 sm:grid-cols-2">
      <div className="sm:col-span-2"><label htmlFor="donor-name" className={labelClass}>Full Name <span className="text-red-700">*</span></label><input id="donor-name" name="fullName" required maxLength={150} autoComplete="name" className={inputClass} /></div>
      <div><label htmlFor="donor-mobile" className={labelClass}>Mobile Number <span className="text-red-700">*</span></label><input id="donor-mobile" name="mobileNumber" type="tel" required maxLength={30} autoComplete="tel" className={inputClass} /></div>
      <div><label htmlFor="donation-amount" className={labelClass}>Donation Amount (LKR) <span className="text-red-700">*</span></label><input id="donation-amount" name="amount" type="number" required min="0.01" max="9999999999.99" step="0.01" inputMode="decimal" className={inputClass} /></div>
      <div><label htmlFor="transfer-date" className={labelClass}>Transfer Date <span className="text-red-700">*</span></label><input id="transfer-date" name="transferDate" type="date" required className={inputClass} /></div>
      <div><label htmlFor="bank-reference" className={labelClass}>Bank / Transfer Reference <span className="font-normal text-muted">(optional)</span></label><input id="bank-reference" name="bankReference" maxLength={150} className={inputClass} /></div>
      <div className="sm:col-span-2"><label htmlFor="donation-note" className={labelClass}>Message / Note <span className="font-normal text-muted">(optional)</span></label><textarea id="donation-note" name="note" maxLength={2000} rows={4} className={`${inputClass} resize-y`} /></div>
