...
export async function AboutSnapshot() {
  const [team, vehicle] = await Promise.all([
    prisma.employeeProfile.findFirst({ where: { isPublicProfile: true, user: { role: "EMPLOYEE", status: "APPROVED" } }, select: { fullName: true }, orderBy: { fullName: "asc" } }),
    prisma.vehicle.findFirst({ where: { isActive: true, isPublic: true }, select: { id: true, name: true, imageUrl: true }, orderBy: [{ displayOrder: "asc" }, { name: "asc" }] }),
  ]);
  const items = [
    { href: "/team", label: "Leadership", title: `${leadershipTeam.length} community leaders`, image: "/images/jws/covers/administration-team.png", icon: <PeopleIcon /> },
    { href: "/team", label: "Our team", title: team?.fullName ?? "A trusted service team", image: "/images/jws/team/group.jpeg", icon: <PeopleIcon /> },
    { href: "/vehicles", label: "Our vehicles", title: vehicle?.name ?? "Ready for community service", image: "/images/jws/covers/vehicles-cover.png", icon: <VehicleIcon /> },
  ];
  return <section aria-labelledby="about-snapshot-title" className="overflow-hidden bg-white px-6 py-16 sm:py-24"><div className="mx-auto max-w-7xl">
    ...
    <div className="mt-10 grid gap-4 sm:grid-cols-3">{items.map((item) => <Link key={item.label} href={item.href} className="group relative min-h-64 overflow-hidden rounded-3xl border border-border bg-light-background focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary">
      {item.image ? <Image unoptimized src={item.image} alt={item.title} fill sizes="(min-width: 640px) 33vw, 100vw" className="object-contain transition duration-500 group-hover:scale-[1.025] motion-reduce:transition-none" /> : <div className="absolute inset-0 flex items-center justify-center bg-[radial-gradient(circle_at_center,rgba(69,232,205,0.28),transparent_55%)] text-foreground/55">{item.icon}</div>}
