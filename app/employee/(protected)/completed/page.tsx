...
export default async function CompletedTasksPage() {
  ...
  const tasks = profileId ? await prisma.taskAssignment.findMany({ where: { employeeId: profileId, status: "COMPLETED" }, orderBy: [{ request: { createdAt: "desc" } }, { requestId: "desc" }, { id: "desc" }], select: { id: true, acceptedAt: true, completedAt: true, request: { select: { requestCode: true, serviceType: true, area: true } } } }) : [];
