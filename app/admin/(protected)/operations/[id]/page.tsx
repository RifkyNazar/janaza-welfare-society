...
export default async function AdminOperationPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ created?: string }> }) {
  await requireAdmin();
  ...
  return <div className="mx-auto max-w-6xl"><BackButton fallbackHref="/admin/operations" /><div className="mt-6 flex flex-col justify-between gap-4 sm:flex-row"><div><p className="text-sm font-semibold text-primary">{operation.taskAssignment?.request.requestCode ?? "Standalone operation"}</p><h1 className="mt-1 text-3xl font-semibold">{operation.title}</h1><p className="mt-2 text-sm text-muted">{operation.isPublished ? `Published ${operation.publishedAt ? date.format(operation.publishedAt) : ""}` : "Private draft"}</p></div><OperationPublishAction published={operation.isPublished} action={setOperationPublished.bind(null, id, !operation.isPublished)} /></div>
  {created && <StatusMessage className="mt-6">Operation created successfully.</StatusMessage>}
  {created && <div className="mt-3 flex flex-wrap gap-3"><Link href="/admin/operations" className="inline-flex min-h-11 items-center rounded-xl border border-border px-4 text-sm font-semibold">Back to Operations</Link><Link href="/admin" className="inline-flex min-h-11 items-center rounded-xl border border-border px-4 text-sm font-semibold">Dashboard</Link><Link href="/operations" className="inline-flex min-h-11 items-center rounded-xl border border-border px-4 text-sm font-semibold">View Website</Link></div>}
