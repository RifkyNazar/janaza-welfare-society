...
export type DonationReviewState = { error?: string; success?: string };
...
export async function reviewDonation(donationId: number, decision: string, _state: DonationReviewState, formData: FormData): Promise<DonationReviewState> {
  ...
  if (reviewNote.length > 191) return { error: "Review note must be 191 characters or fewer." };
  ...
  if (result.count !== 1) return { error: "This donation is no longer pending or was reviewed by another administrator." };
